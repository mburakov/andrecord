...
SLEngineItf CreateAudioEngine(SLObjectItf* object) {
  ...
  if (result != SL_RESULT_SUCCESS) {
    LOG(ERROR, "Failed to create audio engine (%s)", SlResultString(result));
    return NULL;
  }
  result = (**object)->Realize(*object, SL_BOOLEAN_FALSE);
  if (result != SL_RESULT_SUCCESS) {
    LOG(ERROR, "Failed to realize audio engine (%s)", SlResultString(result));
    return NULL;
  }
  SLEngineItf iface;
...
SLRecordItf CreateAudioRecorder(SLEngineItf engine, SLuint32 sample_rate,
  ...
  if (result != SL_RESULT_SUCCESS) {
    LOG(ERROR, "Failed to create audio recorder (%s)", SlResultString(result));
    return NULL;
  }
  ...
  if (result != SL_RESULT_SUCCESS) {
    ...
    (*config_iface)
        ->SetConfiguration(config_iface, SL_ANDROID_KEY_RECORDING_PRESET,
                           &preset, sizeof(preset));
  }
  result = (**object)->Realize(*object, SL_BOOLEAN_FALSE);
  if (result != SL_RESULT_SUCCESS) {
    LOG(ERROR, "Failed to realize audio recorder (%s)", SlResultString(result));
    return NULL;
  }
  SLRecordItf iface;
