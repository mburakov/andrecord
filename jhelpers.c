...
static jobject GetSystemService(JNIEnv* env, jobject activity,
    ...
    do {
        jfieldID service_name_field =
                FIND_OR_BREAK(service_name_field, StaticField, env,
                              activity_class, name, "Ljava/lang/String;");
        service_name = UNEXCEPT(GetStaticObjectField, env, activity_class,
                                service_name_field);
        if (!service_name) {
        ...
        jmethodID get_system_service = FIND_OR_BREAK(
                get_system_service, Method, env, activity_class,
...
int GetBufferConfig(JNIEnv* env, jobject activity, int* sample_rate_out,
    ...
    do {
        ...
        audio_manager = GetSystemService(env, activity, activity_class,
                                         "AUDIO_SERVICE");
        if (!audio_manager) {
        ...
        jmethodID get_property = FIND_OR_BREAK(
                get_property, Method, env, manager_class, "getProperty",
                "(Ljava/lang/String;)Ljava/lang/String;");
        FOR_EACH(void*(*it)[], values) {
            jfieldID property_name_field = FIND_OR_BREAK(
                    property_name_field, StaticField, env, manager_class,
